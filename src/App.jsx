...
const verifiedKeys = ['birthDate', 'job', 'location', 'height', 'asset', 'income', 'education', 'housing', 'car'];
...
function recommendJobStability(job) {
  if (!job) return 3;
...
const VERDICT_EMOJI = { green: '✅', blue: '🔍', orange: '⚠️', amber: '⏸️', red: '🚩' };
...
function analyze(candidate) {
  ...
  return { age, rows, conditionScore, relationScore, trustScore, realityScore, bonusPenalty, flowScore, totalScore, verifiedCount, verdict, label, color, comments, meetCount, meetBonus };
}
                      ...
                      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
                        {(showAllSignals ? sortedSignals : sortedSignals.slice(0, 3)).map((sig) => (
                          <div key={sig.code} style={{ display: 'inline-flex', alignItems: 'center', gap: '4px', background: `var(--${sig.tone}-light)`, color: `var(--${sig.tone})`, padding: '4px 8px', borderRadius: '6px', fontSize: '12px', fontWeight: 600 }}>
                            ...
                            <span style={{ fontSize: '10px', background: 'rgba(255, 255, 255, 0.65)', color: 'inherit', padding: '2px 5px', borderRadius: '4px', marginLeft: '2px' }}>{sig.count}회</span>
                          </div>
                        ))}
                      </div>
                    </div>
    ...
    <div className="sheetBackdrop" onClick={close}>
      <div className="guide" onClick={(e) => e.stopPropagation()}>
        ...
        <Card>
          ...
          <ScoreRule title="정보 확인도" score="15점" desc="확인된 정보 수, 핵심 조건 실물 검증 완료도 (직업 확인 시 추가 가산)" />
